...
export default function ProductDetail() {
  ...
  if (loading) return <div className="container-page py-16 text-center text-brand-muted">{t("common.loading")}</div>;
  if (notFound || !product) {
    return (
      <EmptyState
        icon="🔍"
  ...
  return (
    <div className="container-page py-8 sm:py-12">
      ...
      <div className="grid md:grid-cols-2 gap-10">
        <div className="aspect-square rounded-2xl bg-white border border-brand-border flex items-center justify-center overflow-hidden">
          {product.imageUrl ? (
        ...
        <div>
          ...
          <div className="mt-8 flex items-center gap-4">
            <div className="flex items-center border border-brand-border rounded-full">
              <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-10 h-10 text-lg font-bold text-brand-navy">−</button>
              <span className="w-10 text-center font-semibold">{qty}</span>
              <button onClick={() => setQty((q) => q + 1)} className="w-10 h-10 text-lg font-bold text-brand-navy">+</button>
            </div>
            <button
              onClick={() => {
                addItem(product, qty);
            ...
            >
              {added ? `✓ ${t("product.added")}` : t("product.addToEstimate")}
            </button>
          </div>
          ...
          <a
            ...
            className="mt-4 inline-flex items-center gap-2 text-brand-primary font-semibold hover:underline"
          >
